...
const OBJECTIVE_FEATURE_SETS: FeatureSet[] = [
  [has4OrMoreWords, 4],
  [isBold, -1], 
  [hasAt, -4], 
  [hasParenthesis, -3], 
...
export const extractProfile = (sections: ResumeSectionToLines) => {
  ...
  const [objective, objectiveScores] = getTextWithHighestFeatureScore(
    textItems,
    OBJECTIVE_FEATURE_SETS,
    undefined,
    true
  );
  ...
  return {
    profile: {
      ...
      location,
      url,
      objective:objectiveSection||objectiveSection|| objective,
    },
